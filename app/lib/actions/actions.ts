...
export async function generateDialogues(
  ...
  try {
    ...
    console.log("dialogue data generated by ai: ", dialogueData);
    ...
    console.log("error at ai dialogue generation: ", e);
  }
}
...
export async function createFullVideo(
  ...
  console.log("raw dialogue data: ", dialogueData);
  ...
  try {
    ...
    if (areDialoguesValid) {
      dialogues.map((dialogueData) => {
        ...
        createIndividualVideo(
          jobId,
          dialogue,
          dialogueData.voiceId ?? "",
          dialogueData.templateVideoUrl ?? "",
          videoNumber,
          userId
        );
      });
    } else {
      console.log("error at validating jobs");
    }
  } catch (e) {
    console.log(e);
...
export async function createIndividualVideo(
  jobId: string,
  dialogue: string,
  voiceId: string,
  ...
  try {
    ...
    const audioUrl = await uploadAudioToS3(audioBody, userId);

    const webhookUrl = `${process.env.TEMPORAL_URL}/api/lip-sync/updateVideoStatus`;

    console.log("WEBHOOK URL: ", webhookUrl);

    const syncLabsRequest = await lipSyncVideo(
      audioUrl,
      templateVideoUrl,
      webhookUrl
    );
    ...
    const storedVideoQueryResult = await storeVideoName(
      syncLabsVideoId,
      jobId,
      userId,
      "PENDING",
...
export async function generateChatGPTDialogues(content: PromptFormat) {
  ...
  try {
    ...
    console.log("error at ai generated dialogues: ", e);
  }
}
