  ...
  try {
    ...
    if (areDialoguesValid) {
      dialogues.map((dialogueData) => {
        ...
        createIndividualVideo(jobId, dialogue, dialogueData.voiceId ?? "", dialogueData.templateVideoUrl ?? "", videoNumber, userId);
      });
    } else {
...
export async function createIndividualVideo(jobId: string, dialogue: string, voiceId: string, templateVideoUrl: string, videoNumber: number, userId: string) {
